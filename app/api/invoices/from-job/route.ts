...
export async function POST(req:Request){
    try {
      ...
      return NextResponse.json({ error: "Customer not found" }, { status: 404 });
        ...
        const total = calculateInvoiceTotal(items,parsed.discountAmount,parsed.taxPercent);
      ...
      dueDate: parsed.dueDays ? new Date(Date.now() + parsed.dueDays * 24 * 60 * 60 * 1000) : undefined,
